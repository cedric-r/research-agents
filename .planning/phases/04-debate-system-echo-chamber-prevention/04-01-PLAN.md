...
<task type="auto">
  ...
  <action>
        ...
        while (!empty($running)) {
            foreach ($running as $pid => $info) {
                ...
                if ($reaped > 0 || $reaped === -1) {
                    $result = $this->readTempFile($info['sanitized_name'], $info['name']);
                    if ($result !== null) {
                    ...
                    $this->cleanTempFile($info['sanitized_name'], $info['name']);
                    unset($running[$pid]);
    ...
    Verify the constructor already accepts a Logger. The existing constructor (line ~48-65) takes `Logger $logger` and stores it as `$this->logger`. The canFork() protected method (line ~135) checks `function_exists('pcntl_fork')` which is used by the existing research() method. Both are already present.
